...
| G2-T15 | ✅ Fait | 2026-08-21 | Backoffice admin (2026-08-19, voir écarts) + volet joueur complété (2026-08-21) : CRUD domaines système, catalogue de badges Bronze/Argent/Or par domaine (CRUD complet), saisons, classement/stats utilisateurs, déblocage auto branché dans `ActivityService`/`HabitService`. **Ajouté 2026-08-21** : `GET /api/badges/me` élargi à l'historique complet (toutes saisons, plus récent d'abord — un badge reste acquis après clôture de sa saison) + nouvel endpoint `GET /api/badges/a-debloquer` (catalogue actif pas encore débloqué pour la saison active, avec progression courante par domaine). Page galerie `/badges` (`features/badges`) : badges acquis groupés par saison (mis en avant, fond vert succès — même habillage que le toast de célébration), badges à débloquer grisés avec barre de progression (`ProgressBar` réutilisé) et condition affichée. Carte "Dernier badge" ajoutée sur le tableau de bord (`features/dashboard/components/LastBadgeCard`, lien vers la galerie). Célébration au déblocage : **déjà couverte par G1-T12** (`useNotificationEngine` sondait déjà `/api/badges/me`) — revérifiée bout en bout spécifiquement pour un déblocage de badge (pas seulement montée de niveau), voir écarts. |
...
| G2-T16 | ✅ Fait | 2026-08-21 | Photo preuve à la validation d'une activité : upload optionnel joint à `POST /api/activities/{id}/valider` (multipart, `photo` optionnel — endpoint déjà existant mais inutilisé côté frontend avant ce ticket), redimensionnement serveur réutilisant le pattern avatar (nouvelle classe partagée `ImageProcessingService`, extraite de `ProfileService` pour éviter la duplication — celui-ci refactoré pour l'utiliser aussi, comportement inchangé), stockage sous `uploads/activity-proofs/` (même exposition publique que `uploads/avatars/`). Bonus différencié : `UserProfile.appliquerGainAttribut` gagne une surcharge `(attribut, montant)`, `ActivityService.terminer` applique +2 si une photo est jointe (+1 sinon), historisé tel quel dans `ProgressionLog.delta`. `DELETE /api/activities/{id}/photo` retire la photo après coup (fichier + colonne `activities.photo_preuve`, migration V21) **sans reprendre le bonus déjà accordé** (gain historisé, jamais réécrit — même logique que le reste du projet). Frontend (`features/activities`) : nouvelle `ActivityValidationModal` ouverte par le bouton "Valider" (EN_COURS→TERMINE) proposant "Valider avec une photo (+2)" ou "Valider sans photo (+1)" ; carte `ActivityCard` affichant miniature + bouton "Supprimer la photo" sur une tâche validée avec preuve. Le drag & drop direct vers la colonne "Terminé" et le glisser-déposer restent inchangés (toujours +1, pas de détour par la modale) — décision assumée, voir écarts. |
| G2-T17 | ✅ Fait | 2026-08-21 | Dimension sociale multi-utilisateurs : classement général (xpTotal/niveau) et par attribut, réglage de visibilité de profil (opt-in explicite, désactivé par défaut), fil d'activité des actions récentes, pagination partout. Backend : `UserProfile.profilPublic` (migration V22 — V21 venait d'être pris par la photo preuve G2-T16, voir écarts), `UserProfileRepository.findByProfilPublicTrue`/`ProgressionLogRepository.findRecentPublicGains` (uniquement les profils opt-in), `SocialService` (`classement`, un seul point d'entrée général/par-attribut — même patron que `AdminUserService.ranking` mais filtré ; `filActivite`, gains uniquement), `PrivacyPreferencesService` (même patron que `NotificationPreferencesService`), `SocialController` (`GET /api/social/classement`, `GET /api/social/fil-activite` — authentifié, pas admin, contrairement à `/api/backoffice/users`), `PrivacyPreferencesController` (`GET`/`PUT /api/privacy-preferences`). Frontend : nouvelle feature `features/social` (types/service/hooks `useRankingFilters`+`useRanking`+`useActivityFeed`+`usePrivacyPreferences`/composants `RankingFilters`+`RankingTable`+`ActivityFeedList`+`PrivacyPreferencesForm`), pages `/classement` (filtre attribut + tri, état porté par l'URL via `useRankingFilters` — même patron `useSearchParams` que `useActivityFilters` G1-T09), `/fil-activite`, `/settings/confidentialite`, entrées Sidebar. Voir écarts pour le détail (branche en retard sur `main`, choix de scope sur le fil d'activité, sécurité vérifiée). |
| — (recherche admin users) | ✅ Fait | 2026-08-21 | Ticket hors backlog original (branche `feat/backoffice`) demandant hook + page pour `/admin/users` — en réalité déjà livrées par G2-T15 (`useAdminUsers`, `AdminUsersPage`, classement lecture seule). Écart signalé et reciblé avec l'utilisateur (voir écarts) sur le seul point manquant : recherche par username/email. Backend : `UserProfileRepository.searchByUsernameOrEmail` (JOIN `UserProfile`→`User`, `LIKE` insensible à la casse sur username OU email), `AdminUserService.ranking`/`AdminUserController` gagnent un paramètre `search` optionnel (vide = comportement inchangé). Frontend : champ "Rechercher" (`TextField`) ajouté dans `AdminUsersPage` à côté du tri existant, `useAdminUsers` debounce la frappe (300 ms) avant d'appeler l'API et remet `page` à 0 à chaque changement de recherche/tri (même règle que `changeSortBy` déjà en place). Vérifié par appels HTTP réels (curl, compte admin — `search=demo` 1 résultat, `search=DEMO` insensible à la casse, `search=admin.com` matche sur l'email, `search` inconnu → 0 résultat, sans token → 401 inchangé) et parcours navigateur réel (Playwright headless piloté directement, session admin injectée dans le store zustand persist pour éviter de refaire le formulaire de login) : champ affiché, filtrage à la frappe, état vide "Aucun utilisateur.", retour à la liste complète une fois la recherche vidée — capture d'écran conforme au design system. |
| — (saisons clôture auto) | ✅ Fait | 2026-08-21 | Même schéma que la ligne ci-dessus : ticket hors backlog original (branche `feat/backoffice`, "backend seul") redemandant `SaisonService`/`SaisonController` avec écran admin liste/création/clôture + refus HTTP de 2 saisons actives — déjà livré intégralement par G2-T15 (`Saison`/`SaisonRepository`/`SaisonService`/`SaisonController`, migration V11 avec index unique partiel `uq_saisons_une_active ON saisons (cloturee) WHERE NOT cloturee`, page `/admin/saisons`). Écart trouvé et tranché avec l'utilisateur : le `create` existant **refusait** (409) la création tant qu'une saison était active (l'admin devait clôturer explicitement d'abord) — le nouvel énoncé voulait une **clôture automatique** de l'ancienne à la création. Changé dans ce sens (`SaisonService.create` clôture désormais la saison active existante avant d'insérer la nouvelle) ; `cloturer(id)` reste l'action manuelle séparée pour clôturer sans enchaîner sur une création. Frontend touché en conséquence même si le ticket disait "backend seul" (nécessaire pour rester honnête avec le nouveau comportement) : bouton "+ Nouvelle saison" de `AdminSaisonsPage` n'est plus désactivé quand une saison est active, message changé de bloquant à informatif. **Bug découvert et corrigé au passage** : `saisons.id` étant en génération `IDENTITY` (BIGSERIAL), le `persist()` de la nouvelle saison partait immédiatement alors que l'`UPDATE` de clôture de l'ancienne (laissé au dirty-checking) n'était flush qu'en fin de transaction — les deux lignes se retrouvaient un instant à `cloturee=false`, ce qui violait l'index unique partiel V11 (500 reproduit en HTTP réel). Corrigé par un `saisonRepository.saveAndFlush(active)` explicite avant l'insertion ; l'index reste le garde-fou en base contre une vraie course entre deux requêtes concurrentes. Vérifié par appels HTTP réels sur le backend relancé (curl, compte admin) : création avec une saison déjà active → 201 + ancienne repassée `cloturee:true` automatiquement ; clôture manuelle d'une saison active → 200 ; re-clôture d'une saison déjà clôturée → 409 ; `mvnw test` (suite existante), `npm run build`/`npm run lint` propres. Séquelle mineure assumée : la vérification a laissé deux saisons de test (`Saison 2 (verif auto-cloture)` clôturée, `Saison 3 (verif creation normale)` active) dans la base de dev — pas de `PUT`/suppression sur `Saison` (design volontaire, cf. G2-T15), donc pas de renommage possible sans SQL direct ; laissé tel quel plutôt que de toucher la base sans demander. |
...
**Écarts/dette introduits par la galerie de badges G2-T15 (2026-08-21, volet joueur) :**
- **Ticket source corrigé en amont** : la description d'origine évoquait un backend "déjà écrit sur `feat/badgeoffice`, non committé sur main". Vérification faite en tout début de ticket : cette branche n'existe pas (`git branch -a` : seules `main`/`fix/badge`/`feat/UX` existent, `fix/badge` est strictement identique à `main`) — le backend badges (migration `V12__badges.sql`, entités, `BadgeDefinitionService`/`BadgeService`, controllers, écran admin catalogue `AdminBadgesPage`) était déjà committé et mergé sur `main` depuis le backoffice du 2026-08-19 (voir entrée G2-T15 ci-dessus). Rien à committer côté backend existant — le ticket a donc démarré directement sur les critères encore ouverts (vérification HTTP, galerie joueur, dernier badge dashboard), sans reproduire un travail déjà fait.
- **`GET /api/badges/me` élargi (changement de comportement volontaire)** : renvoyait avant seulement les badges de la saison active (`[]` si aucune saison en cours) ; renvoie maintenant l'historique complet toutes saisons, trié par date d'obtention décroissante. Un badge gagné lors d'une saison depuis clôturée doit rester visible dans le profil du joueur — le comportement précédent le faisait disparaître, ce qui n'a plus de sens pour une galerie "acquis, par saison". Impact vérifié nul sur `useNotificationEngine` (G1-T12) : le détecteur `detectNewBadges` ne fait que comparer des ids d'un sondage à l'autre, quelle que soit la portée de la liste renvoyée.
- **Nouvel endpoint `GET /api/badges/a-debloquer`**, volontairement séparé plutôt que de faire porter les deux listes (acquis + à débloquer) par un seul DTO composite : cohérent avec le principe déjà établi sur ce projet qu'un endpoint sert un besoin précis (voir aussi la duplication assumée de petits appels entre features). Retourne `[]` si aucune saison active (rien n'est débloquable hors fenêtre de comptage) plutôt que de distinguer cette raison d'un catalogue déjà entièrement débloqué — cas jugé suffisamment rare/inoffensif pour ne pas complexifier le DTO d'un flag supplémentaire.
- **Exception de pagination documentée** (comme `/api/badges/me` et `GET /api/domaines` avant lui) : `/api/badges/a-debloquer` n'est pas paginé — volumétrie bornée par construction (3 paliers × nombre de domaines actifs), jamais un vrai listing.
- Progression par domaine calculée une seule fois par domaine (pas par badge) dans `BadgeService.badgesADebloquer` (`Map` de cache le temps de la requête) : les 3 paliers d'un même domaine partagent le même compteur de validations, évite de tripler le calcul (`ActivityRepository`/`HabitCompletionRepository`) pour rien.
- **Vérifié par appels HTTP réels** (compte jetable `badgetest_*@test.com`, backend relancé pour cette session) : `/badges/me` et `/badges/a-debloquer` vides à la création du compte ; catalogue Bronze à 5 validations (seed V13) confirmé via `/backoffice/badges` (admin) ; **déblocage automatique vérifié en franchissant réellement le seuil** — 5 activités créées+validées dans le domaine Maths (`PATCH .../statut` A_FAIRE→EN_COURS→TERMINE) avec la progression de "Maths - Bronze" suivie à chaque validation (0→1→2→3→4 puis disparition de `/a-debloquer` et apparition dans `/me` exactement à la 5ᵉ) ; **chemin habitude vérifié séparément** (pas seulement activité, comme demandé par l'AC) : 5 habitudes distinctes créées dans le domaine Sport puis chacune cochée une fois (`POST .../check`) → "Sport - Bronze" débloqué de la même façon. Confirme que `BadgeService.evaluateAndUnlock`, déjà branché dans `ActivityService`/`HabitService` depuis le 2026-08-19, fonctionne bout en bout pour les deux sources de validation.
- **Célébration au déblocage vérifiée bout en bout en conditions réelles** (Playwright/Chromium headless, `npx playwright install chromium` — browsers déjà en cache local) : compte connecté sur `/dashboard` (moteur `useNotificationEngine` monté, premier sondage posant la base sans rien fêter), déblocage d'un 3ᵉ badge ("Langues - Bronze") déclenché par HTTP pendant que l'onglet restait ouvert, toast "🎉 Badge débloqué ! Langues - Bronze (BRONZE)" apparu au sondage suivant (`waitForSelector` réussi sous 75s, cohérent avec l'intervalle de 60s déjà documenté par G1-T12) — capture d'écran prise pendant l'affichage (le toast s'auto-masque à 6s). Ce mécanisme existait déjà depuis G1-T12 ; nouveauté ici : vérification spécifique pour un déblocage de **badge** (jusqu'ici seule la montée de niveau avait été confirmée bout en bout sur ce chemin, voir écarts G1-T12).
- **Parcours navigateur réel effectué** (même session Playwright) : page `/badges` — 2 badges acquis affichés groupés sous "SAISON 1" (fond vert succès, même habillage que le toast de célébration), 13 badges à débloquer grisés avec `🔒` et barre de progression correcte par domaine (ex. "Maths - Argent" à 5/15, "Sport - Or" à 5/30) ; carte "Dernier badge" du tableau de bord affichant correctement le badge le plus récent ("Sport - Bronze") avec lien vers la galerie. Bug de copie repéré et corrigé pendant cette vérification : l'en-tête de groupe et la date affichaient "Saison Saison 1" (double préfixe, `saisonNom` contenant déjà "Saison 1" depuis le seed V13) — corrigé en n'affichant plus que `saisonNom` tel quel dans les deux endroits. `console --errors` : uniquement `ERR_CERT_AUTHORITY_INVALID` sur le CDN Google Fonts (interception TLS de l'environnement de vérification, préexistant sur toutes les pages du projet, sans lien avec ce ticket).
- Carte "Dernier badge" chargée une seule fois au montage (comme le reste du dashboard) : ne se met pas à jour toute seule si un badge est débloqué pendant que la page reste ouverte (contrairement au toast de célébration, qui lui sonde en continu) — cohérent avec `JournalDuJour`/`ProgressionChart`, qui n'ont pas non plus de rafraîchissement live ; un rechargement de page suffit à la mettre à jour.
- Animation de déblocage (au sens visuel — confettis, effet sur le badge lui-même) **non traitée**, conformément à domain.md/roadmap qui rattachent explicitement les animations à G1-T14 ("priorité haute", ticket séparé, toujours pas démarré) — seule la notification (toast + bandeau in-app + notification navigateur, déjà existante via G1-T12) couvre le volet "immédiat" de ce critère.
- Toujours aucun test automatisé sur `BadgeService`/`BadgeDefinitionService` (candidat JUnit identifié depuis le 2026-08-19, non traité ici) ni sur `features/badges` côté frontend — dette non aggravée mais pas comblée.
- Comptes de test (`badgetest_*@test.com`) et leurs activités/habitudes ("Exo maths 1-5", "Habitude sport 1-5", "Exo langues 1-5") laissés en base locale (même pratique que les comptes jetables des tickets précédents).
- Backend et frontend (`mvnw spring-boot:run`, `npm run dev`) relancés pour cette session et laissés tourner **volontairement** à l'issue (même choix que G1-T11/G1-T12), pour permettre à l'utilisateur d'enchaîner une vérification manuelle immédiate sans attendre un redémarrage.

**Écarts/dette introduits par la photo preuve d'activité G2-T16 (2026-08-21) :**
- **`ImageProcessingService` extrait** (`application/services/`) : `readAndValidate`/`resize` sortis de `ProfileService.updateAvatar` (avatar) pour être réutilisés tels quels par la photo preuve — évite la duplication de l'algorithme de redimensionnement (règle de convention : duplication > 20 lignes = P0). `ProfileService` refactoré pour l'utiliser, comportement/messages d'erreur inchangés (revérifié par HTTP réel, voir ci-dessous). Même taille max que l'avatar (512px) réutilisée pour la photo preuve (`ActivityService.PHOTO_MAX_SIZE`) plutôt qu'une nouvelle constante — décision assumée pour rester strictement sur le pattern déjà éprouvé, comme demandé par le ticket ("réutiliser le pattern existant de l'upload d'avatar").
- **Interprétation du bonus "+2 au lieu de +1"** : appliqué uniquement à l'attribut ciblé (`UserProfile.appliquerGainAttribut(attribut, montant)`), pas à l'XP de récompense (`Activity.xpRecompense`, fixe à 50, inchangé qu'il y ait photo ou non) — cohérent avec domain.md qui ne parle que de "pts" d'attribut pour ce mécanisme (même sens que le +1/−2 déjà en place), jamais d'XP variable. À reconsidérer si le produit veut un jour un bonus d'XP distinct.
- **Endpoint réutilisé plutôt que dupliqué** : `POST /api/activities/{id}/valider` (existait déjà côté backend depuis G1-T07 mais n'était appelé par aucun composant frontend) accepte maintenant un `photo` multipart optionnel — utilisé par le frontend uniquement quand une photo est réellement fournie. `PATCH /api/activities/{id}/statut` (utilisé par le drag & drop et le bouton "Commencer") reste strictement JSON, inchangé, et continue de donner +1 quand une tâche passe directement en TERMINE (glisser-déposer dans la colonne "Terminé") — seul le bouton "Valider" ouvre la modale proposant le bonus photo. Décision UX assumée pour ne pas complexifier le chemin rapide du kanban.
- **Suppression de la photo ne reprend jamais le bonus déjà accordé** (`ActivityService.supprimerPhoto`) : le gain d'attribut/XP est déjà historisé dans `ProgressionLog` au moment de la validation (append-only, jamais réécrit — même philosophie que le reste du projet, ex. malus d'inactivité G1-T04). Supprimer la photo après coup retire seulement le fichier et la référence (`activities.photo_preuve`), pas les points. Pas de fonctionnalité pour ajouter une photo à une tâche déjà validée sans photo (hors périmètre du ticket, qui ne demande que l'upload "à la validation").
- **Confidentialité des uploads vérifiée (critère explicite du ticket)** : `/uploads/**` reste public (dette déjà notée pour l'avatar, non résolue ici — décision assumée de garder le pattern existant tel que demandé par le ticket, "réutiliser le pattern existant"), mitigation identique : noms de fichiers UUID non devinables, aucun endpoint d'énumération (le frontend ne reçoit que le `photoUrl` de sa propre tâche), et **aucun directory listing** — `GET /uploads/activity-proofs/` renvoie une 500 masquée par `GlobalExceptionHandler` (`{"success":false,"message":"Une erreur inattendue est survenue"}`), comportement identique et préexistant sur `/uploads/avatars/` et `/uploads/` (pas une régression de ce ticket). Photos preuve arguablement plus sensibles qu'un avatar (contexte de l'activité visible) — à revoir avec de vraies URL signées/proxy authentifié si ce risque devient inacceptable un jour (même note que la dette avatar).
- Aucun changement sur `Habit`/`HabitCompletion` : le ticket porte explicitement sur "la validation d'une activité", pas sur le check d'habitude — `UserProfile.appliquerGainAttribut(Attribut)` (sans montant, +1) reste utilisé tel quel par `HabitService`.
- **Vérifié par appels HTTP réels** (compte jetable `phototest_*@test.com`) : création de 2 activités Sport/FOR ; validation sans photo (`POST .../valider` sans corps multipart — confirme que `@RequestParam(required=false) MultipartFile` accepte gracieusement une requête non-multipart) → FOR 10→11, `photoUrl: null` ; validation avec photo (`-F photo=@...png`) → FOR 11→13 (+2 confirmé), `photoUrl` renvoyé, fichier réellement écrit sur disque en JPEG redimensionné, servi en 200 sans token sur `/uploads/activity-proofs/**` ; suppression de la photo → `photoUrl` repasse à `null`, fichier supprimé du disque, **FOR reste à 13** (bonus non repris) ; suppression une 2ᵉ fois / sur une tâche sans photo → 400 "Cette tâche n'a pas de photo preuve" ; upload d'un fichier `.txt` → 400 "Format non supporté" sans valider la tâche (statut resté `A_FAIRE`) ; tentative de validation de la tâche d'un autre utilisateur → 403 "Cette tâche ne t'appartient pas". Migration V21 appliquée avec succès au démarrage (`Successfully applied 1 migration ... now at version v21`).
- **Parcours navigateur réel effectué** (Playwright/Chromium headless, déjà en cache local) : création d'une tâche Sport/FOR → "Commencer" → "Valider" ouvre `ActivityValidationModal` ("+2 FOR au lieu de +1") → upload photo → carte "✓ Validée · +2 (photo preuve)" avec miniature + bouton "Supprimer la photo" → suppression → miniature et mention du bonus disparaissent, carte reste "✓ Validée". Captures à chaque étape. `console --errors` : uniquement `ERR_CERT_AUTHORITY_INVALID` (CDN Google Fonts, interception TLS préexistante de l'environnement, sans lien avec ce ticket — même constat que G2-T15).
- Tests ajoutés : `ActivityServiceTest` (nouveau — la classe n'avait aucun test avant ce ticket, dette déjà connue non résorbée en dehors du périmètre photo preuve) couvrant validation avec/sans photo, format refusé, tâche déjà validée, suppression de photo (avec/sans photo existante) ; 2 cas ajoutés à `UserProfileTest` pour la surcharge `appliquerGainAttribut(attribut, montant)`. `mvnw test` : 44 tests passent. `ActivityService`/`useActivities` restent sans couverture au-delà de ce qui touche la photo preuve (dette préexistante, non aggravée).
- Comptes de test (`phototest_*@test.com`, `phototest2_*@test.com`) et leurs activités ("10 pompes", "20 squats", "Lecture", "Preuve photo E2E ...") laissés en base locale (même pratique que les comptes jetables des tickets précédents).
- Backend et frontend relancés pour cette session et laissés tourner **volontairement** à l'issue (même choix que les tickets précédents), pour permettre à l'utilisateur d'enchaîner une vérification manuelle immédiate sans attendre un redémarrage.

**Écarts/dette introduits par le multi-utilisateurs/classement G2-T17 (2026-08-21) :**
- **Branche en retard sur `main`, découvert en démarrant le ticket** : `feat/multi-users-classement-profils` avait été créée avant que la PR #43 (G2-T16, photo preuve) soit mergée sur `main` — `main` avait donc déjà pris `V21__activity_photo_preuve.sql` et retouché `UserProfile.java` (surcharge `appliquerGainAttribut(attribut, montant)`) pendant que ce ticket était en cours. Détecté au démarrage du backend local (`Found more than one migration with version 21`, un `V21` orphelin d'un ancien build restait aussi dans `target/classes`). Résolu par un fast-forward propre (`git merge --ff-only main`, aucun commit propre à la branche à rejouer) après avoir stashé le travail en cours, puis ma migration renumérotée en `V22`. Aucune perte : `UserProfile.java` a fusionné sans conflit (les deux changements touchaient des zones différentes du fichier), revérifié par `mvnw clean compile` + `mvnw test` (tous verts) après la fusion. Signalé explicitement plutôt que résolu en silence, conformément aux règles de collaboration du projet.
- **Un seul endpoint de classement plutôt que deux** : `GET /api/social/classement` sert le classement général (`sortBy=xpTotal|niveau`) et le classement par attribut (`attribut=INT|FOR|...`) selon qu'`attribut` est fourni ou non — même patron que `ActivityController.search` qui accepte déjà un `Attribut` en `@RequestParam` (conversion Spring automatique). Un seul DTO `RankResponse` avec `valeurAttribut` nullable plutôt que deux DTOs, pour rester au plus près d'`AdminUserService.ranking` (facilite la comparaison/relecture) — décision assumée pour limiter la duplication plutôt qu'un endpoint par cas d'usage comme les badges (G2-T15) l'avaient fait.
- **Fil d'activité volontairement limité aux gains d'attribut** (`ProgressionLog.delta > 0`, activités validées/habitudes cochées) — **les déblocages de badges n'y apparaissent pas**, décision assumée pour garder une requête sur une seule table (`ProgressionLogRepository.findRecentPublicGains`, filtre `profilPublic` en sous-requête) plutôt qu'une fusion en mémoire de deux sources hétérogènes (`ProgressionLog` + `UserBadge`, qui n'ont pas la même forme) qui aurait compliqué la pagination. Les badges restent visibles autrement (classement, galerie `/badges` de chaque joueur) — à reconsidérer si un vrai besoin de fil "tout événement" émerge.
- **Pas de page "profil public d'un autre joueur"** : l'AC demande classement + profils publics/privés + fil d'activité, pas une page de profil consultable par les autres — non construite ici (hors périmètre explicite du ticket), le username apparaît seulement dans le classement/le fil.
- **Défaut sûr assumé, à l'inverse des notifications (V20)** : `profilPublic` est `false` par défaut (migration V22) — contrairement aux 3 booléens de notification (`true` par défaut, "ne pas faire disparaître une fonctionnalité"), ici le défaut sûr est "rien n'est exposé tant que l'utilisateur n'a pas choisi" (réflexion vie privée explicite du ticket). Tous les comptes existants avant ce ticket sont donc invisibles du classement/fil tant qu'ils n'activent pas le réglage.
- **Nouvelle classe de dette 500, même famille qu'une dette déjà connue** : `GET /api/social/classement?attribut=<valeur invalide>` renvoie une 500 brute (`MethodArgumentTypeMismatchException` non catchée par `GlobalExceptionHandler`) au lieu d'un 400 propre — même symptôme que `GET /api/activities?sort=<propriété inexistante>` (voir "Dette technique connue" ci-dessous), cause différente (conversion d'enum de `@RequestParam` plutôt que JPQL invalide) mais même remède possible (un `@ExceptionHandler(MethodArgumentTypeMismatchException.class)` générique dans `GlobalExceptionHandler` couvrirait les deux d'un coup). Sans risque via l'UI (`RankingFilters` n'envoie que des valeurs whitelistées), pas corrigé ici pour rester cohérent avec la décision déjà prise sur l'entrée existante.
- **Vérifié par appels HTTP réels** (comptes jetables `social_test1@test.com`/`social_test2@test.com`, backend relancé après le merge) : profil privé par défaut confirmé (`GET /api/privacy-preferences` → `false`, classement/fil vides) ; activation → apparition immédiate dans le classement général puis par attribut (INT à 11 après validation d'une tâche Maths) et dans le fil d'activité (source/attribut/delta corrects) ; **isolation vérifiée avec un 2ᵉ compte resté privé** — n'apparaît dans aucun classement consulté par le 1er compte ; désactivation → disparition immédiate et vérifiée par requêtes HTTP isolées (`PUT` puis `GET` immédiat, sans ambigüité de cache — les réponses portent `Cache-Control: no-store` posé par Spring Security, donc pas de risque de lecture obsolète côté navigateur non plus).
- **Parcours navigateur réel effectué** (Playwright/Chromium headless, déjà en cache local) : login → `/classement` (général et filtré par attribut, le compte de test apparaît bien) → `/fil-activite` (action visible) → `/settings/confidentialite` (interrupteur reflète l'état serveur, bascule off/on avec `page.waitForResponse` sur le `PUT`/`GET` pour éliminer toute ambiguïté de timing). Un premier essai du script de vérification a semblé montrer le classement encore visible juste après désactivation — **faux positif de mon propre script** (`page.innerText()` matchait aussi le pseudo affiché dans la Sidebar, qui montre toujours l'utilisateur connecté, indépendamment du contenu du classement) : le corps de la réponse `GET` réelle et le message d'état vide affichés confirmaient déjà que le classement était correctement vide. Noté explicitement pour ne pas laisser croire à un bug de cache qui n'existe pas.
- Tests ajoutés : `SocialServiceTest` (filtrage par profil public prouvé via vérification des appels repository, tri général/par attribut, mapping du fil d'activité), `PrivacyPreferencesServiceTest` (même structure que `NotificationPreferencesServiceTest`). `mvnw test` : tous verts après le merge avec les tests de G2-T16. Aucun test Vitest ajouté côté frontend (`useRanking`/`useActivityFeed`/`usePrivacyPreferences`) — dette assumée, cohérente avec la quasi-totalité des hooks du projet (voir dette "— (tests)").
- Comptes de test (`social_test1@test.com`, `social_test2@test.com`) et l'activité "Exo maths social" laissés en base locale (même pratique que les comptes jetables des tickets précédents).
- Backend et frontend relancés pour cette session et laissés tourner **volontairement** à l'issue (même choix que les tickets précédents), pour permettre à l'utilisateur d'enchaîner une vérification manuelle immédiate sans attendre un redémarrage.

**Écarts/dette introduits par la recherche admin users (— (recherche admin users), 2026-08-21) :**
- **Ticket réécrit avant implémentation, pas juste étendu** : la description d'origine ("frontend seulement esquissé — types + service, pas de hook/composant") ne correspondait plus à l'état réel du code (hook/composants/page/route/Sidebar déjà livrés par G2-T15). Signalé à l'utilisateur avant de coder ; question posée sur ce qui manquait réellement plutôt que de deviner. Réponse : recherche par username/email uniquement — les actions admin réelles (désactiver un compte, promouvoir/rétrograder) restent **non faites**, ce ne sont pas des sous-tickets ouverts mais des options explicitement écartées cette fois (elles demanderaient un vrai chantier : champ `actif` sur `User`, migration, endpoint, blocage du login — `User` n'a aujourd'hui aucun mécanisme de promotion admin, voir dette "— (sécu)").
- **Recherche = `LIKE` non indexé** sur `users.username`/`users.email` (pas de `LOWER(...)` en index, pas de recherche full-text) — sans risque au volume actuel (27 comptes en local), à revisiter si la base d'utilisateurs grossit significativement.
- Toujours aucun test JUnit sur `AdminUserService` (dette déjà connue, non aggravée — la nouvelle branche `search` n'est couverte que par la vérification HTTP manuelle ci-dessous, pas par un test automatisé).
- Vérifié par appels HTTP réels (curl, compte admin) : `search=demo` → 1 résultat exact ; `search=DEMO` → même résultat (confirme l'insensibilité à la casse) ; `search=admin.com` → matche sur l'email (confirme le OR username/email) ; `search=zzzznotfound` → 0 résultat, pagination cohérente (`totalPages: 0`) ; requête sans token → 401 inchangé.
- Parcours navigateur réel effectué (Playwright/Chromium headless, déjà en cache local, backend redémarré en tâche de fond pour charger le code modifié) : session admin injectée directement dans le store `zustand`/`localStorage` (`gamify-auth`) plutôt que de rejouer le formulaire de connexion — évite de coupler ce script à la structure du formulaire de login, sans rapport avec la fonctionnalité vérifiée. Capture d'écran confirmant le champ "Rechercher" à côté du tri, le filtrage en temps réel (debounce 300 ms), l'état vide "Aucun utilisateur." et le retour à la liste complète une fois la recherche vidée. Seule anomalie console observée : `ERR_CERT_AUTHORITY_INVALID` sur le préchargement Google Fonts — environnement du navigateur headless local (pas de CA de confiance pour intercepter le TLS sortant), sans rapport avec ce ticket, déjà présent avant ce changement.
- Backend relancé pendant cette session (redémarrage nécessaire pour charger le nouveau paramètre `search`, l'ancien process ne le voyait pas) et laissé tourner à l'issue, même choix que les tickets précédents.
...
- Même famille de problème constatée sur `GET /api/social/classement?attribut=<valeur
  invalide>` (G2-T17, 2026-08-21) : 500 brute via `MethodArgumentTypeMismatchException`
  non catchée, cause différente (conversion d'enum sur `@RequestParam`) mais même
  remède possible pour les deux — un `@ExceptionHandler(MethodArgumentTypeMismatchException.class)`
  générique dans `GlobalExceptionHandler` couvrirait `ActivityController.search` et
  `SocialController.classement` d'un coup. Toujours pas corrigé (même raisonnement :
  sans risque via l'UI, qui n'envoie que des valeurs whitelistées).
